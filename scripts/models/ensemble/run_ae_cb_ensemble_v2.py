"""
AE-MLP + CatBoost Ensemble V2: 3x AE-MLP + 1 CatBoost

Extends V1 ensemble by adding AE-MLP with Market-Neutral target (v9-mkt-neutral).
Models predict different targets but ensemble uses z-score normalization to unify scales.
...
Usage:
    ...
    python scripts/models/ensemble/run_ae_cb_ensemble_v2.py

    # With backtest
    python scripts/models/ensemble/run_ae_cb_ensemble_v2.py --backtest --topk 10

    # Compare with V1 ensemble (3 models)
    python scripts/models/ensemble/run_ae_cb_ensemble_v2.py --exclude-mkt-neutral
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='AE-MLP + CatBoost Ensemble V2: 3x AE-MLP + 1 CatBoost',
    )
    ...
    parser.add_argument('--exclude-mkt-neutral', action='store_true',
                        help='Exclude mkt-neutral model (run V1 3-model ensemble for comparison)')
    ...
    for name, config in MODEL_CONFIGS.items():
        ...
        if name == 'AE-MLP-v7' and args.exclude_v7:
            continue
        if name == 'AE-MLP-v9' and args.exclude_v9:
            continue
        if name == 'AE-MLP-mkt-neutral' and args.exclude_mkt_neutral:
            continue
        if name == 'CatBoost' and args.exclude_catboost:
            continue
    ...
    print("=" * 70)
    print(f"AE-MLP + CatBoost Ensemble V2: {n_models} Models")
    print("=" * 70)
    ...
    if args.learn_weights:
        ...
        # Use the first non-mkt-neutral dataset for label
        label_source = None
        for name in model_names:
            if 'mkt-neutral' not in active_models[name]['handler']:
                label_source = name
                break
    ...
    # Summary
    print("\n" + "=" * 70)
    print(f"ENSEMBLE V2 COMPLETE ({n_models} models)")
    print("=" * 70)
    print("Weights:")
    for name in model_names:
        r = ic_results[name]
    ...
    if args.backtest:
        if not hasattr(args, 'handler'):
            args.handler = "ensemble_v2"
        run_ensemble_backtest(pred_ensemble, args, time_splits)

        print("\n" + "=" * 70)
        print("ENSEMBLE V2 BACKTEST COMPLETE")
        print("=" * 70)
