"""
AE-MLP + AE-MLP (mkt-neutral) + AE-MLP-V9R + CatBoost + TCN-FiLM Ensemble (V6)
...
Five-model ensemble:
  ...
  5. TCN-FiLM (alpha300 + core macro, Optuna-tuned)

Based on run_ae_cb_ensemble_v5.py with TCN-FiLM macro-conditioned model added.

Usage:
    python scripts/models/ensemble/run_ae_cb_ensemble_v6.py
    python scripts/models/ensemble/run_ae_cb_ensemble_v6.py --ensemble-method rank_mean
    python scripts/models/ensemble/run_ae_cb_ensemble_v6.py --stacking
    python scripts/models/ensemble/run_ae_cb_ensemble_v6.py --backtest --topk 10 --n-drop 2
"""
...
def load_tcn_film_model(model_path, device):
    """Load TCN-FiLM model from checkpoint."""
...
def create_tcn_film_dataset(symbols, time_splits, nday, include_valid=False):
    """Create alpha300 DatasetH for TCN-FiLM predictions."""
    ...
    handler = HandlerClass(
        ...
        fit_end_time=time_splits.get('train_end', time_splits['test_end']),
    )
...
def predict_with_tcn_film(model, dataset, macro_df, macro_cols,
                          segment="test", d_feat=5, step_len=60,
                          macro_lag=1, batch_size=2048, device=None):
    """Generate predictions with TCN-FiLM model."""
    if device is None:
    ...
    macro = prepare_macro(features.index, macro_df, macro_cols, macro_lag)
    stock = features.values.reshape(-1, d_feat, step_len)

    ds = TCNMacroDataset(stock, macro, labels.values)
    ...
    return pd.Series(pred, index=features.index, name='score')
...
def run_cv_evaluation(models, handlers, tcn_film_model, tcn_film_ckpt,
                      macro_df, macro_cols, args, symbols, MODEL_CONFIG, device):
    """Evaluate ensemble IC on CV folds."""
    model_names = list(MODEL_CONFIG.keys())
    display_names = {k: cfg['display'] for k, cfg in MODEL_CONFIG.items()}

    print("\n" + "=" * 80)
    print("CROSS-VALIDATION EVALUATION (5-Model Ensemble V6)")
    print("=" * 80)
    ...
    for key in model_names:
        ...
        if cfg['type'] == 'tcn_film':
            test_datasets[key] = create_tcn_film_dataset(
                symbols, test_time, args.nday, include_valid=True)
        else:
    ...
    # Test predictions
    test_preds = {}
    batch_size = tcn_film_ckpt.get('best_params', {}).get('batch_size', 2048)
    for key in model_names:
        cfg = MODEL_CONFIG[key]
        if cfg['type'] == 'catboost':
            test_preds[key] = predict_catboost_raw(models[key], test_datasets[key], "test")
        elif cfg['type'] == 'tcn_film':
            test_preds[key] = predict_with_tcn_film(
                tcn_film_model, test_datasets[key], macro_df, macro_cols,
                segment="test", batch_size=batch_size, device=device)
        else:
    ...
    for fold in CV_FOLDS:
        ...
        for key in model_names:
            cfg = MODEL_CONFIG[key]
            if cfg['type'] == 'tcn_film':
                fold_datasets[key] = create_tcn_film_dataset(
                    symbols, fold_time, args.nday, include_valid=True)
            else:
        ...
        for key in model_names:
            ...
            if cfg['type'] == 'catboost':
                ...
                val_preds[key] = predict_with_tcn_film(
                    tcn_film_model, fold_datasets[key], macro_df, macro_cols,
                    segment="valid", batch_size=batch_size, device=device)
            else:
    ...
    print("\n" + "=" * 80)
    print("CV EVALUATION COMPLETE (5-Model Ensemble V6)")
    print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(
        description='AE-MLP + AE-MLP(mkt-neutral) + AE-MLP-V9R + CatBoost + TCN-FiLM Ensemble (V6)',
    )
    ...
    parser.add_argument('--tcn-film-model', type=str,
                        default=str(MODEL_SAVE_PATH / 'tcn_film_optuna_sp500_core_5d_best.pt'),
    ...
    parser.add_argument('--macro-set', default='core', choices=['minimal', 'core'],
                        help='Macro feature set for TCN-FiLM (default: core)')
    parser.add_argument('--macro-lag', type=int, default=1,
                        help='Macro feature lag for TCN-FiLM (default: 1)')
    parser.add_argument('--gpu', type=int, default=0,
                        help='GPU device for TCN-FiLM (default: 0)')
    ...
    parser.add_argument('--ae-weight', type=float, default=0.20,
                        help='AE-MLP V7 weight (default: 0.20)')
    parser.add_argument('--ae-mn-weight', type=float, default=0.20,
                        help='AE-MLP mkt-neutral weight (default: 0.20)')
    parser.add_argument('--ae-v9r-weight', type=float, default=0.20,
                        help='AE-MLP V9 rolling weight (default: 0.20)')
    parser.add_argument('--cb-weight', type=float, default=0.20,
                        help='CatBoost weight (default: 0.20)')
    parser.add_argument('--tcn-weight', type=float, default=0.20,
                        help='TCN-FiLM weight (default: 0.20)')
    ...
    print("=" * 80)
    print("AE-MLP-V7 + AE-MLP-MN + AE-MLP-V9R + CatBoost + TCN-FiLM Ensemble (V6)")
    print("=" * 80)
    ...
    for key, cfg in MODEL_CONFIG.items():
        if cfg['type'] == 'tcn_film':
            ...
            tcn_cfg = ckpt['config']
            print(f"    {cfg['display']} config: n_chans={tcn_cfg['n_chans']}, "
                  f"layers={tcn_cfg['num_layers']}, kernel={tcn_cfg['kernel_size']}, "
                  f"film_hidden={tcn_cfg['film_hidden']}")
            if 'valid_ic' in ckpt:
                print(f"    {cfg['display']} saved IC: valid={ckpt['valid_ic']:.4f}, test={ckpt.get('test_ic', 0):.4f}")
            continue
    ...
    # [4] Load macro data (for TCN-FiLM)
    print("\n[4] Loading macro data for TCN-FiLM...")
    macro_df = load_macro_df()
    macro_cols = get_macro_cols(args.macro_set)
    available = [c for c in macro_cols if c in macro_df.columns]
    print(f"    Macro features: {args.macro_set} ({len(available)} features)")
    ...
    for key, cfg in MODEL_CONFIG.items():
        if cfg['type'] == 'tcn_film':
            print(f"    Creating alpha300 dataset for {cfg['display']}...")
            datasets[key] = create_tcn_film_dataset(
                symbols, time_splits, args.nday, include_valid=True)
        else:
    ...
    for key, cfg in MODEL_CONFIG.items():
        if cfg['type'] == 'ae_mlp':
            ...
            tcn_film_model, tcn_film_ckpt = load_tcn_film_model(cfg['path'], device)
            models[key] = tcn_film_model
            print(f"    TCN-FiLM loaded on {device}")
    ...
    if args.skip_cv:
        ...
        run_cv_evaluation(models, handlers_dict, tcn_film_model, tcn_film_ckpt,
                          macro_df, macro_cols, args, symbols, MODEL_CONFIG, device)
    ...
    for key, cfg in MODEL_CONFIG.items():
        ...
        if cfg['type'] == 'ae_mlp':
            ...
            pred = predict_with_tcn_film(
                tcn_film_model, datasets[key], macro_df, macro_cols,
                segment="test", batch_size=batch_size, device=device)
        pred_dict[key] = pred
    ...
    print("\n    +" + "=" * 70 + "+")
    print("    |" + " " * 10 + "Information Coefficient (IC) Comparison (V6)" + " " * 14 + "|")
    print("    +" + "=" * 70 + "+")
    ...
    print("    +" + "-" * 70 + "+")
    print(f"    |  {'Ensemble (V6)':<20s} | {ens_ic:>10.4f} | {ens_std:>10.4f} | {ens_icir:>10.4f} |")
    print("    +" + "=" * 70 + "+")
    ...
    # V5 comparison hint
    v5_models = {k: v for k, v in ic_results.items() if k != 'tcn'}
    if v5_models:
        v5_preds = {k: pred_dict[k] for k in v5_models}
        v5_ens = ensemble_predictions(v5_preds, args.ensemble_method,
                                       {k: weights[k] for k in v5_models} if weights else None)
        v5_ic, _, v5_icir, _ = compute_ic(v5_ens, label)
        print(f"\n    V5 (4-model) Ensemble IC: {v5_ic:.4f}, ICIR: {v5_icir:.4f}")
        print(f"    V6 (5-model) Ensemble IC: {ens_ic:.4f}, ICIR: {ens_icir:.4f}")
        v6_delta = ens_ic - v5_ic
        print(f"    V6 vs V5 IC delta: {v6_delta:+.4f}")

    # Summary
    print("\n" + "=" * 80)
    print("ENSEMBLE V6 ANALYSIS COMPLETE")
    print("=" * 80)
    ...
    print(f"{'Ensemble V6':15s} IC: {ens_ic:.4f} (ICIR: {ens_icir:.4f})")
    print("=" * 80)

    # Backtest
    if args.backtest:
        run_ensemble_backtest(pred_ensemble, args, time_splits, model_name="Ensemble_V6")

        print("\n" + "=" * 80)
        print("ENSEMBLE V6 BACKTEST COMPLETE")
        print("=" * 80)
