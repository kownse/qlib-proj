...
使用自定义 PyTorch Transformer 模型，带有友好的训练日志。

使用方法:
    # 基础训练
    python scripts/models/deep/run_transformer_v2.py --handler alpha300

    # 使用 IC 损失
    python scripts/models/deep/run_transformer_v2.py --handler alpha300 --use-ic-loss

    # 完整训练 + 回测
    python scripts/models/deep/run_transformer_v2.py --handler alpha300 --stock-pool sp500 --backtest
"""
...
from models.deep.transformer_model import TransformerModel, create_model, PRESET_CONFIGS
...
def main():
    parser = argparse.ArgumentParser(description='Transformer V2 Training')
    ...
    # 创建模型
    print("\n[*] Creating model...")
    model = create_model(preset=args.preset, **model_kwargs)
    ...
    model_filename = f"transformer_v2_{args.handler}_{args.stock_pool}_{args.nday}d_{timestamp}.pt"
    ...
    if args.backtest:
        ...
        def load_model_func(path):
            return TransformerModel.load(str(path), GPU=args.gpu)
