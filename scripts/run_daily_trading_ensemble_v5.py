#!/usr/bin/env python
"""
每日交易脚本 (Ensemble V5) - 4模型集成

模型:
  1. AE-MLP (alpha158-enhanced-v7)
  2. AE-MLP mkt-neutral (v9-mkt-neutral)
  3. AE-MLP V9R (alpha158-enhanced-v9, trained from 2015)
  4. CatBoost (catboost-v1)

默认 Stacking Weights: AE-MLP=0.300, AE-MLP-MN=0.200, AE-MLP-V9R=0.250, CatBoost=0.250
...
使用方法:
    python scripts/run_daily_trading_ensemble_v5.py
    python scripts/run_daily_trading_ensemble_v5.py --skip-download
    python scripts/run_daily_trading_ensemble_v5.py --predict-only
    python scripts/run_daily_trading_ensemble_v5.py --send-email
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Daily trading script (Ensemble V5) - 4-model ensemble',
    )
    add_common_trading_args(parser)

    # Override defaults for V5
    ...
    parser.add_argument('--ae-v9r-model', type=str,
                        default='my_models/ae_mlp_cv_alpha158-enhanced-v9_sp500_5d_20_from_2015.keras',
                        help='Path to AE-MLP V9R (rolling 2015) model')
    ...
    parser.add_argument('--ae-weight', type=float, default=0.300,
                        help='AE-MLP weight (default: 0.300)')
    parser.add_argument('--ae-mn-weight', type=float, default=0.200,
                        help='AE-MLP mkt-neutral weight (default: 0.200)')
    parser.add_argument('--ae-v9r-weight', type=float, default=0.250,
                        help='AE-MLP V9R weight (default: 0.250)')
    ...
    weights = {
        ...
        'ae_v9r': args.ae_v9r_weight,
        'cb': args.cb_weight,
    }

    print("=" * 70)
    mode = "LIVE PREDICTION MODE" if args.predict_only else "BACKTEST MODE"
    print(f"DAILY TRADING SCRIPT (ENSEMBLE V5) - {mode}")
    print("=" * 70)
    ...
    print(f"\n{'='*70}")
    print("ENSEMBLE V5 TRADING SCRIPT COMPLETED")
    print(f"{'='*70}")

    # Step 10: 发送邮件报告
    send_trading_email(
        args, trading_details,
        model_info="AE-MLP-V7 + AE-MLP-MN + AE-MLP-V9R + CatBoost (Ensemble V5, zscore_weighted)",
    )
