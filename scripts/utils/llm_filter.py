...
def _load_cache(symbol: str) -> dict | None:
    """Load cached analysis for a symbol, or None if not cached or expired."""
    ...
    if cache_file.exists():
        try:
            with open(cache_file, "r") as f:
                data = json.load(f)
            # Check expiration
            analyzed = data.get("analyzed_date")
            if analyzed:
                age = (datetime.now() - datetime.strptime(analyzed, "%Y-%m-%d")).days
                if age > CACHE_TTL_DAYS:
                    print(f"    [LLM] {symbol.upper()} → EXPIRED ({age} days old, limit={CACHE_TTL_DAYS})")
                    return None
            return data
