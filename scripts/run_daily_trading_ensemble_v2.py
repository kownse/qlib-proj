...
def run_live_prediction(
    ...
    print(f"\n{'='*70}")
    print("LIVE TRADING PREDICTIONS (Ensemble V2: 2x AE-MLP + CatBoost)")
    print(f"{'='*70}")
    ...
    pred_path = output_dir / f"ensemble_v2_predictions_{latest_date.strftime('%Y%m%d')}.csv"
    ...
    rec_path = output_dir / f"ensemble_v2_recommendations_{latest_date.strftime('%Y%m%d')}.csv"
...
def run_trading_backtest(
    ...
    print(f"\n{'='*70}")
    print("TRADING BACKTEST (Ensemble V2: 2x AE-MLP + CatBoost)")
    print(f"{'='*70}")
    ...
    for freq, (report_df, positions) in portfolio_metric_dict.items():
        ...
        report_path = output_dir / f"ensemble_v2_daily_trading_report_{freq}.csv"
...
def main():
    ...
    if args.send_email and trading_details:
        ...
        if recent_details:
            ...
            subject = f"Ensemble V2 Trading Report: {start_date} to {end_date}"
