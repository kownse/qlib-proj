"""
Enhanced DataHandler V5 - Stock-specific features only (no macro).

Purpose: Test whether macro features cause overfitting in V4.

Changes from V4:
- Removed ALL macro features (62 features)
- Keep only stock-specific technical features (19 features)
- Optional: minimal lagged macro regime indicators (off by default)

V4 Problem: Validation IC 0.43, Test IC -0.03 (severe overfitting)
V5 Goal: Better generalization with stock-specific features only

Features (19 total):
- Alpha158 minimized: 9 features
...
# Default macro features path (only used if macro_mode != "none")
DEFAULT_MACRO_PATH = PROJECT_ROOT / "my_data" / "macro_processed" / "macro_features.parquet"


class Alpha158_Enhanced_V5(DataHandlerLP):
    """
    Enhanced Alpha158 V5 - Stock-specific features only.

    Removes all macro features to test generalization.
    Total: 19 features (vs 83 in V4)
    ...
    Optional macro_mode:
    - "none" (default): No macro features
    - "minimal": Only 3 lagged regime indicators (VIX regime, credit stress, yield curve)
    """

    # Minimal macro features - only regime indicators, 1-day lagged
    MINIMAL_MACRO_FEATURES = [
        "macro_vix_level",        # VIX level for regime
        "macro_credit_stress",    # Credit stress indicator
        "macro_yield_curve_slope", # Yield curve for regime
    ]
    ...
    def __init__(
        ...
        macro_mode: str = "none",  # "none", "minimal"
        macro_lag: int = 1,  # Days to lag macro features
        macro_data_path: Union[str, Path] = None,
        **kwargs,
    ):
        """
        Initialize Enhanced V5 DataHandler.

        Args:
            macro_mode: "none" (default) or "minimal" (3 lagged features)
    ...
    def get_feature_config(self):
        """Get feature config with V5 stock-specific features only."""
        fields = []
        names = []
    ...
    def process_data(self, with_fit: bool = False):
        """Override process_data to optionally add minimal lagged macro features."""
        super().process_data(with_fit=with_fit)

        # Only add macro if mode is "minimal"
        if self.macro_mode == "minimal" and self._macro_df is not None:
            self._add_minimal_macro_features()

    def _add_minimal_macro_features(self):
        """Add minimal lagged macro regime indicators."""
        try:
            available_cols = [c for c in self.MINIMAL_MACRO_FEATURES if c in self._macro_df.columns]

            if not available_cols:
                print("Warning: No minimal macro features available")
                return
    ...
    def _load_macro_features(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            print(f"Loaded macro features for minimal mode: {df.shape}")
            return df
        except Exception as e:
